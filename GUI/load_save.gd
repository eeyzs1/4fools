...
func _on_quick_save_game_button_pressed() -> void:
	if Playerstats.role_name == "uninitialized":
		return
	if len(Gamerecord.records_info) == 0:
		create_new_record()
	Gamerecord.update_record(Gamerecord.records_info[0]["record_index"])
	var target_record = record_list.get_children()[1]
	var record_info = Gamerecord.records_info[0]
	if record_info["level_mode"]:
...
func _on_delete_record_button_pressed() -> void:
	if len(Gamerecord.records_info) == 0:
		return
	if not record_index:
		record_index = Gamerecord.records_info[-1]["record_index"]
	var records = record_list.get_children()
	if len(records) > 1:
		var index = Gamerecord.delete_record(record_index) + 1
